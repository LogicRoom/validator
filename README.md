...
| `withMiddleware`              | Method          | this adds a middleware that can be used to access and manipulate the value coming into `onChange` before it gets set to `value` (e.g. `v => v.toUpperCase()`)           |                                        | `(newValue: T) => T`                                             |
